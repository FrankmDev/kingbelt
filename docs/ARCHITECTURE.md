...
Todo adaptador valida el catálogo normalizado antes de exponerlo. `assertValidCatalog()` falla con rutas y códigos concretos para identidades, relaciones, opciones, variantes, dinero, inventario, medios y colecciones; el adaptador demo y el preflight Shopify ejecutan esa frontera sobre el catálogo completo. El producto principal de una PDP se mapea de forma estricta y un producto presente pero inválido propaga un error de proveedor/catálogo, nunca una falsa 404. En listados secundarios runtime (home, colección, índice y relacionados), cada `ProductSummary` pasa por el mapper estricto dentro de una frontera que captura exclusivamente `ShopifyCatalogMappingError` y `CatalogValidationError`: omite ese nodo, registra solo tipo de recurso, handle seguro y clase de error, y continúa hasta el límite solicitado. Excepciones inesperadas, errores de red y fallos globales no se convierten en listas vacías. El preflight conserva la validación global estricta y falla por el mismo nodo. Los providers request-scoped comparten una caché breve de datos anónimos (30 s en producción); la IP del comprador no forma parte de esa caché. La caché queda acotada a 512 recursos por instancia para que handles dinámicos no provoquen crecimiento de memoria sin límite. Un fallo transitorio del proveedor (red, timeout, 5xx, 429) puede servir la última respuesta válida del mismo recurso (stale-if-error) durante un máximo de 15 minutos; un error de mapping, validación o configuración no reutiliza stale. El catálogo completo y el preflight exigen el SKU comercial. La ficha runtime puede usar `shopify-variant-<id>` como identificador técnico estable si falta, sin presentarlo, incluirlo en JSON-LD ni certificarlo como SKU comercial; ese prefijo queda reservado.
...
Las imágenes viven una sola vez en `Product.images`. `primaryImageId`, `ProductVariant.imageId` y los grupos por valor de opción solo contienen referencias. Con Color, `ProductVariant.imageId` conserva la identidad de la imagen efectiva devuelta por Storefront, ya validada contra la portada; sin Color, se usa `ProductVariant.image` si existe. Esa identidad no demuestra procedencia ni asignación explícita en Admin. Los grids reciben `ProductSummary` y nunca el array de variantes.
...
- si el Cart remoto existe pero incumple una invariante determinista que impide representarlo, el BFF responde `409` con `cart_unrecoverable`, conserva el Cart ID en la sesión y bloquea checkout. Solo el comando same-origin `reset` iniciado por la persona elimina esa referencia local; no muta ni intenta revertir el Cart remoto. Timeouts, 429 y 5xx siguen siendo errores transitorios y no recomiendan reset.
