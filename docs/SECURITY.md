...
- El job `quality` de GitHub Actions ejecuta `bun run validate` con `COMMERCE_SOURCE=demo` y sin secretos Shopify. Los pull requests no reciben credenciales de producción. Las Actions oficiales van fijadas por SHA, con `contents: read` y `persist-credentials: false`.
