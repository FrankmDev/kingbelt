...
La auditoría real del 21 de agosto de 2026 confirma `cdn.shopify.com` como host exacto de imágenes y que `Product.images` ya contiene una familia propia por color con la convención `MODELO_COLOR_01/02/03`. Esa media nativa es la única autoridad de galerías. Preflight y runtime exigen una sola familia y exactamente tres imágenes únicas numeradas por cada valor de Color; nunca incorporan archivos externos ni reparten imágenes por posición. La imagen efectiva que Storefront devuelve en `ProductVariant.image` para todas las tallas de un Color debe corresponder a la portada (`COLOR_01`); el mapper no sustituye una imagen incorrecta o ausente. Storefront puede aplicar un fallback a una imagen de producto, por lo que esta comprobación no certifica la asignación explícita en Admin. Los metafields `kingbelt.*` de copy y especificaciones pueden caer a campos nativos si aún no están visibles en Storefront. No se inventa material, ancho ni copy comercial. Un fallo transitorio de obtención sin catálogo previo no se renderiza; con uno válido, se sirve el último conocido (stale-if-error). Un error de mapping o validación en un recurso principal falla cerrado y no usa stale; en listados secundarios runtime, solo el summary inválido conocido se omite con un warning servidor sanitizado. El preflight sigue validando todo el catálogo y falla globalmente por ese mismo summary.
...
| `Product.images` | con Color: una familia nativa `MODELO_COLOR_01/02/03` por valor; preflight y runtime exigen exactamente tres imágenes únicas; la imagen efectiva de Storefront en `ProductVariant.image` debe ser `COLOR_01` en todas las tallas; sin Color: `mediaGroups = []` y portada de `featuredImage` | `images`, `primaryImageId`, `mediaGroups` e `imageId` de variante |
...
La extensión puede ser JPG, PNG, WebP, AVIF o GIF y Shopify puede añadir un UUID después del número. El nombre se normaliza sin tildes, mayúsculas ni separadores; el color debe ocupar el sufijo completo de la familia para que `Cuero` no coincida con `Cuero oscuro`, y debe existir un prefijo no vacío de modelo/familia antes del color. El token de secuencia literal es de dos dígitos. Preflight y runtime rechazan familias ausentes, duplicadas, ambiguas, incompletas o con una secuencia distinta de `01/02/03`. La imagen efectiva que Storefront devuelve para todas las tallas de un Color debe ser igual a `MODELO_COLOR_01`.
...
- an existing Cart that fails deterministic integrity mapping returns `409 cart_unrecoverable`, keeps the session Cart ID, blocks checkout, and exposes an explicit same-origin `reset` action;
- `reset` deletes only the Astro Session reference after user confirmation; it does not mutate or roll back the remote Shopify Cart;
...
`Product.images` es la única fuente autoritativa. Para cada valor de `Color`, el mapper busca exactamente una familia cuyo nombre termine en el color normalizado y la ordena por sufijo numérico. Preflight y runtime exigen exactamente tres imágenes únicas `01`, `02` y `03`; `01` se convierte en la portada del color. La imagen efectiva que Storefront devuelve en `ProductVariant.image` para todas las variantes de ese color debe corresponder a esa portada. El mapper compara `Image.id` de la misma consulta de producto y no sustituye una imagen incorrecta o ausente por la portada inferida. Un mismatch o una variante de Color sin imagen efectiva falla cerrado. Storefront no expone la procedencia del fallback, de modo que comprobar una asignación explícita sigue siendo una verificación manual de Shopify Admin. El Cart usa únicamente `merchandise.image` (`ProductVariant.image` efectivo); no hay segundo fallback a `product.featuredImage`.

Referencia oficial fijada a Storefront API 2026-07: [`ProductVariant.image`](https://shopify.dev/docs/api/storefront/2026-07/objects/ProductVariant) documenta expresamente el fallback a una imagen del producto; [`Product.featuredImage`](https://shopify.dev/docs/api/storefront/2026-07/objects/Product) equivale funcionalmente a `images(first: 1)`; [`Image`](https://shopify.dev/docs/api/storefront/2026-07/objects/Image) define la identidad y metadatos visibles, no la procedencia administrativa de la asignación.
...
- [ ] Si tiene Color: la imagen efectiva de Storefront de todas las variantes del color es igual a la portada `COLOR_01`; verificar aparte en Admin la asignación explícita cuando sea necesaria.
...
- Fail-closed proporcional en runtime: el producto principal de una PDP y los recursos estructurales deben cumplir el contrato o propagar error. En listados secundarios, un summary individual inválido se omite únicamente si el fallo es `ShopifyCatalogMappingError` o `CatalogValidationError`; se registra un warning sin payload, stack, GID completo, token ni PII. Errores de configuración (`ShopifyConfigurationError`) y fallos del proveedor no se convierten en contenido vacío. El preflight sigue siendo estricto para el catálogo completo.
...
- El adaptador no degrada a un «catálogo vacío» silencioso por configuración inválida o caída de Storefront: un producto o colección inexistente (`null`) se traduce a `undefined`; un producto principal presente e inválido, o un fallo de API o de configuración, se propaga. Solo un summary secundario conocido como inválido puede omitirse.
