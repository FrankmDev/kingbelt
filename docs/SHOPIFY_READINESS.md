...
La consulta real del 18 de agosto de 2026 confirma `cdn.shopify.com` como host exacto de imágenes, ya autorizado en la política de imagen y la CSP. Los metafields `kingbelt.*` de copy y especificaciones pueden caer a campos nativos si aún no están visibles en Storefront. `custom.kingbelt_color_galleries` es la fuente estructurada de galerías por color: el preflight la exige en productos con opción Color y nunca reparte por posición, filename, URL ni por huecos entre portadas. El runtime de ficha usa esas referencias solo cuando las `MediaImage` ya pertenecen a `product.images` de ese producto; no incorpora media de otro producto. Si el metafield no llega, llega incompleto para un color o apunta a archivos ajenos, completa ese color con la media nativa que comparte la familia de archivo de `variant.image`. Si el metafield llega con tipo incorrecto, falla cerrado. No se inventa material, ancho ni copy comercial. Un fallo transitorio de obtención sin catálogo previo no se renderiza; con uno válido, se sirve el último conocido (stale-if-error). Un error de mapping o validación falla cerrado, no usa stale y no omite el producto del listado.
...
| media de producto | con opción Color y `custom.kingbelt_color_galleries` publicado: exactamente tres `MediaImage` por color (preflight); runtime: esas tres si pertenecen al producto, si no la familia nativa de `variant.image`; sin opción Color: `mediaGroups = []` y portada de `featuredImage` | `images`, `primaryImageId`, `mediaGroups` e `imageId` de variante |

`reference`, `summary`, `badge`, material, ancho y hebilla/acabado se resuelven con los metafields `kingbelt.*` cuando están publicados para Storefront. Si aún no lo están, el importador usa campos nativos: `handle` como referencia, `description` como resumen, título de colección si falta descripción y título de producto como `alt`. `ProductVariant.sku` no tiene fallback: Storefront debe devolver el SKU comercial no vacío; si falta, el mapping y `shopify:preflight` fallan. Para Color, el preflight toma `custom.kingbelt_color_galleries` como única fuente de grupos de tres imágenes: no infiere por filename, URL, alt ni posición. El runtime usa ese metafield solo si las tres `MediaImage` ya están en el producto; si no llegan o son de otro producto, completa el color con la familia nativa de `variant.image`. Un tipo incorrecto sigue fallando. No se inventa copy comercial, material, medidas ni códigos de variante. No se importa coste, margen, tags administrativos ni HTML sin sanear.
...
| Product | `custom.kingbelt_color_galleries` | `list.metaobject_reference` | exactamente una referencia por valor de Color | products with Color option en preflight | preflight: ninguno; runtime: media nativa del producto si el metaobject no le pertenece | `Product.mediaGroups` |
...
`custom.kingbelt_color_galleries` es la fuente autoritativa del preflight para relacionar colores con imágenes. Cada valor de `Color` tiene exactamente un metaobject y tres `MediaImage` ordenadas: posición 1, portada; posición 2, detalle; posición 3, contexto. El orden de `images` en el metaobject es el que configura la persona en Shopify Admin; no se reordena por filename, ID, URL, alt ni posición global en `product.images`. La imagen principal del producto es la posición 1 del primer color según el orden de la opción Color, no el orden accidental del metafield. `variant.image` debe ser `images[0]` de su color; si falta o apunta a otra imagen, el mapping estructurado falla. `Product.mediaGroups` conserva las tres referencias sin copiar objetos. En preflight, las imágenes referenciadas que no vengan en `product.images` se incorporan al producto y se deduplican por `image.id`. En runtime no se incorporan archivos de otro producto: si las tres `MediaImage` no están en ese producto, el color se completa con la media nativa que comparte la familia de archivo de su `variant.image`.

Si el metafield llega vacío, tiene un color de más o de menos, no es `MediaImage`, no tiene dimensiones o repite una imagen, el mapping estructurado falla. El preflight no infiere por filename, URL, alt text, posición ni orden de variantes. Nunca se deben repartir imágenes por posición, orden de `Product.images` ni por huecos entre portadas. El preflight también falla si el metafield no llega por Storefront. El runtime no reparte `product.images` por posición: parte de `variant.image` y, si hace falta, de su familia nativa. Un producto sin opción Color no exige el metafield y conserva `mediaGroups = []`; su imagen principal sale de `featuredImage`. Cada archivo debe tener ID y URL únicos, `alt` no vacío y contextual, y dimensiones positivas conocidas.
...
- Imágenes: una sola vez en `Product.images`. Con opción Color, el preflight toma `custom.kingbelt_color_galleries`: cada color exige un metaobject y tres `MediaImage` ordenadas. El `id` del grupo es el del metaobject. `primaryImageId` es la portada del primer color y `variant.imageId` coincide con la portada de su color (§11). El runtime usa esas referencias solo si pertenecen al producto; si no, completa el color con la familia nativa de `variant.image`. Sin opción Color, `mediaGroups = []` y la imagen principal sale de `featuredImage`. El host real `cdn.shopify.com` está autorizado de forma exacta en `imagePolicy.transformableHosts`, `publicSecurityConfig.remoteImageHosts` y CSP; no se usan comodines. El render aplica `width`/`height`, `srcset`, `sizes` y `loading`/`fetchpriority` sin JavaScript de cliente.
...
3. `bun run shopify:release-gate` — orquesta validate + session + preflight + cart smoke + HTTP del deployment. No certifica Admin ni pagos.
